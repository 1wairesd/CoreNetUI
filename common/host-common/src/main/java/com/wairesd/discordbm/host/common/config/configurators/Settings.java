...
public class Settings {
    ...
    public static String getNettyIp() {
        Object v = getConfigValue(ROOT + ".netty.ip", "127.0.0.1");
        return String.valueOf(v);
    }
    ...
    // MySQL //
    private static <T> T getMySQLOption(String path, T defaultValue, Function<Object, T> caster) {
        Object value = getConfigValue(ROOT + ".mysql." + path, defaultValue);
        return value != null ? caster.apply(value) : defaultValue;
    }

    private static boolean getMySQLOption(String path, boolean defaultValue) {
        return getMySQLOption(path, defaultValue, v -> v instanceof Boolean ? (Boolean) v : defaultValue);
    }

    private static String getMySQLOption(String path, String defaultValue) {
        return getMySQLOption(path, defaultValue, v -> v instanceof String ? (String) v : defaultValue);
    }

    private static int getMySQLOption(String path, int defaultValue) {
        return getMySQLOption(path, defaultValue, v -> v instanceof Number ? ((Number) v).intValue() : defaultValue);
    }

    public static boolean isMySQLEnabled() {
        return getMySQLOption("enabled", false);
    }

    public static String getMySQLHost() {
        return getMySQLOption("mysql.host", "localhost");
    }

    public static int getMySQLPort() {
        return getMySQLOption("mysql.port", 3306);
    }

    public static String getMySQLDatabase() {
        return getMySQLOption("mysql.database", "DiscordBM");
    }

    public static String getMySQLUsername() {
        return getMySQLOption("mysql.username", "root");
    }

    public static String getMySQLPassword() {
        return getMySQLOption("mysql.password", "password");
    }

    public static String getMySQLParams() {
        return getMySQLOption("mysql.params", "?useSSL=false&serverTimezone=UTC");
    }
